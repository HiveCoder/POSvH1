...
export const usePOSStore = create<POSStore>((set, get) => ({
  ...
  currency: storage.getItem('currency') || 'PHP',
  ...
  fetchPosProfile: async () => {
    try {
      const cached = IS_WEBSITE_MODE ? null : localStorage.getItem('posProfile');
      if (cached) {
        ...
        storage.setItem('currency', profile.currency || 'PHP');
      ...
      localStorage.setItem('posProfile', JSON.stringify(combinedProfile));
      set({ 
        posProfile: combinedProfile, 
        profileLoading: false,
        currency: combinedProfile.currency || 'PHP'
      });
      storage.setItem('currency', combinedProfile.currency || 'PHP');
      if (IS_WEBSITE_MODE) {
        storage.setItem('currency', 'PHP');
        storage.setItem('currencySymbol', '₱');
        set({ currency: 'PHP', currencySymbol: '₱' });
      }
  ...
  fetchCategories: async () => {
    try {
      ...
      localStorage.setItem('menuCategories', JSON.stringify(courses));
  ...
  fetchCustomerGroups: async () => {
    const cached = localStorage.getItem('customerGroups');
    if (cached) {
    ...
    localStorage.setItem('customerGroups', JSON.stringify(names));
  },

  fetchTerritories: async () => {
    const cached = localStorage.getItem('territories');
    if (cached) {
    ...
    localStorage.setItem('territories', JSON.stringify(names));
  },
