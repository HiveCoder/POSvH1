...
import { getWebsiteOrders } from '../../lib/website-mock';
...
> = (set, get) => ({
  ...
  fetchOrders: async (page = 1) => {
    try {
      ...
      if (IS_WEBSITE_MODE) {
        const key = orderSearchQuery.trim().toLowerCase();
        ...
        const pageOrders = filtered.slice(limitStart, limitStart + ITEMS_PER_PAGE);

        set({
          orders: pageOrders,
          pagination: {
            currentPage: page,
            hasNextPage: limitStart + ITEMS_PER_PAGE < filtered.length,
            itemsPerPage: ITEMS_PER_PAGE,
          },
          orderLoading: false,
        });
      ...
      const posProfile = localStorage.getItem('posProfile');
  ...
  selectOrder: async (order) => {
    try {
      ...
      if (IS_WEBSITE_MODE) {
        const localOrder = (getWebsiteOrders() as any[]).find((item) => item.name === order.name);
        const items = (localOrder?.items || []) as POSInvoiceItem[];
        set({
  ...
  updateOrderStatus: async (orderId: string, status: POSInvoice['status']) => {
    try {
      ...
      if (IS_WEBSITE_MODE) {
        const orders = getWebsiteOrders() as POSInvoice[];
        const updated = orders.map((order) =>
          order.name === orderId ? { ...order, status } : order
        );
        localStorage.setItem('pos_web_demo_orders', JSON.stringify(updated));
