...
import { IS_WEBSITE_MODE, ROUTER_BASENAME } from '../lib/platform';
...
const Header = () => {
  ...
  const handleLogout = async () => {
    try {
      await logout();
      if (IS_WEBSITE_MODE) {
        window.location.href = ROUTER_BASENAME || '/';
      } else {
      ...
      <div className="flex items-center justify-between h-16 px-6">
        ...
        <div className="flex items-center gap-4">
          <Button
            variant="outline"
            size="sm"
            className="hidden md:inline-flex items-center gap-2 border-slate-300"
