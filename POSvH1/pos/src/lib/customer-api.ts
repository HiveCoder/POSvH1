...
import { addWebsiteCustomer, getWebsiteCustomers } from './website-mock';
...
export async function addCustomer(
  ...
  if (IS_WEBSITE_MODE) {
    const created = addWebsiteCustomer(customerData);
    return {
      data: {
        customer_name: created.customer_name,
        mobile_number: created.mobile_number,
        customer_group: customerData.customer_group || 'Individual',
...
export async function searchCustomers(search: string, limit = 5) {
  ...
  if (IS_WEBSITE_MODE) {
    ...
    return getWebsiteCustomers()
      .filter((customer) =>
        customer.customer_name.toLowerCase().includes(key) ||
        customer.mobile_number.includes(key) ||
